	...
	public void init(double sampleRate, double freq, int len) {
		...
		for (int i=0; i <= M; i++) {
			if (i >= (M+1-pulseLength)/2 && i < M+1-(M+1-pulseLength)/2) {
				double sinc = (GAIN * Math.sin((j - pulseLength/2)))/ (j - pulseLength/2);
				double blackman = 0.42 - 0.5 * Math.cos((2 * Math.PI * j) / pulseLength) + 0.08 * Math.cos((4 * Math.PI * j) / pulseLength);
				if (j == pulseLength/2) {
					xcoeffs[i] = 2 * Math.PI * blackman;
				} else {

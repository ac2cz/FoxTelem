...
//			analyzeModules(rad, none, none, DisplayModule.DISPLAY_UW); // add the experiment header to the top
	...
	protected void parseRawBytes(String data[][], CanPacketRawTableModel radTableModel) {
		...
		String[][] rawData = new String[data.length][UwCanPacket.MAX_PACKET_BYTES-UwCanPacket.ID_BYTES+3]; // +3 fields for ID, HEX ID LEN
		for (int i=0; i<data.length; i++) {
			// ID String
			int id = UwCanPacket.getIdFromRawBytes(Integer.valueOf(data[data.length-i-1][3]),Integer.valueOf(data[data.length-i-1][4]),
					Integer.valueOf(data[data.length-i-1][5]),Integer.valueOf(data[data.length-i-1][6]));
			int len = UwCanPacket.getLengthFromRawBytes(Integer.valueOf(data[data.length-i-1][3]),Integer.valueOf(data[data.length-i-1][4]),
					Integer.valueOf(data[data.length-i-1][5]),Integer.valueOf(data[data.length-i-1][6]));
			for (int k=0; k< 3; k++)
				try {
					if (k<3) // key

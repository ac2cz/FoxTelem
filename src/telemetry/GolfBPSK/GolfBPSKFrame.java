...
/**
	...
	public class GolfBPSKFrame extends Frame {
		...
		public GolfBPSKHeader getHeader() { return (GolfBPSKHeader)header; }
		...
		public void addNext8Bits(byte b) {
			...
			if (numberBytesAdded < telemFormat.getInt(TelemFormat.HEADER_LENGTH)) {
				...
				if (fox != null) {
					...
					if (frameLayout == null) {
						if (Config.debugFrames)
							Log.errorDialog("ERROR","GOLF ID: " + header.id + " Type: " + header.getType() + " frame layout missing. Decode not possible.\n"
									+ "Turn off Debug Frames to prevent this message in future.");
						else
							Log.println("GOLF ID: " + header.id + " Type: " + header.getType() + " frame layout missing. Decode not possible.");
						corrupt = true;
						return;
...
//					initPayloads(header.id, header.getType());
					if (payload[0] == null) {
						if (Config.debugFrames)
							Log.errorDialog("ERROR","GOLF ID: " + header.id + " Type: " + header.getType() + " not valid. Decode not possible.\n"
									+ "Turn off Debug Frames to prevent this message in future.");
						else
							Log.println("GOLF ID: " + header.id + " Type: " + header.getType() + " not valid. Decode not possible.");
		...
		private void initPayloads(GolfBPSKHeader header, FrameLayout frameLayout) {
